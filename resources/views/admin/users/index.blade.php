...
<head>
    ...
    <style>
        ...
        .badge-role {
            padding: 0.5rem 0.8rem; border-radius: 999px; font-size: 0.75rem; font-weight: 600;
        }
        .primary-btn {
            background: linear-gradient(135deg, #184b82 0%, #1d6fd6 100%); border: none; border-radius: 0.85rem; padding: 0.7rem 1rem; font-weight: 600;
        }
            ...
            <div class="main-panel">
                <div class="panel header-panel mb-4">
                    <div class="d-flex justify-content-between align-items-center flex-wrap gap-3">
                        ...
                        <a href="{{ route('admin.users.create') }}" class="btn primary-btn text-white">Tambah User</a>
                    </div>
                </div>
                            ...
                            <tbody>
                                @forelse($users as $user)
                                    <tr>
                                        ...
                                        <td>
                                            @php
                                                $roleClass = match($user->role) {
                                                    ...
                                                    'coach' => 'bg-info-subtle text-info',
                                        ...
                                        <td>
                                            @php
                                                $statusClass = $user->status === 'active' ? 'bg-success-subtle text-success' : 'bg-warning-subtle text-warning';
                                            @endphp
                                            <span class="badge-role {{ $statusClass }}">{{ ucfirst($user->status) }}</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center text-muted py-4">Belum ada user yang terdaftar.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
